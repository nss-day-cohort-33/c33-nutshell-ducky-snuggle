...
const eventComponent = {
  ...
  createEventForm: function() {
    ...
    submitBtn.addEventListener("click", event => {
      ...
      const newEvent = utilityFunc.createEventObj(
        userId,
        eventName,
        eventDate,
        eventLocation
  ...
  createEventComponent: function(eventObj) {
    ...
    deleteBtn.addEventListener("click", () => {
      console.log("delete");
      API.deleteFromApi("events", eventObj.id).then(data => {
        // API.getFromApi("event", userId).then(RENDER.insertComponent)
  ...
  createEditForm: function(eventObj) {
    return `
            <fieldset>
                ...
                <input type="text" name="event-name-edit" id="event-name-edit" value="${
                  eventObj.event_name
                }">
                <br>
                <label for="event-date-edit">Date of event:</label>
                <input type="date" name="event-date-edit" id="event-date-edit" value="${
                  eventObj.event_date
                }">
                <br>
                <label for="event-location-edit">Location of event:</label>
                <input type="text" name="event-location-edit" id="event-location-edit" value="${
  ...
  editedComponentToDom: function(eventToEdit, editForm, eventObj) {
    ...
    document.querySelector("#save-event-btn").addEventListener("click", () => {
      ...
      const eventLocation = document.querySelector("#event-location-edit")
        .value;

      const editedEvent = utilityFunc.createEventObj(
        userId,
        eventName,
        eventDate,
        eventLocation
      ...
      API.updateApi("events", editedEvent).then(() => {
        RENDER.getAndDisplayEvents(eventObj);
      });
    });
  }
